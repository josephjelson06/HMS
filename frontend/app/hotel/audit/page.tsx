...
import { hotelAuditApi } from "@/lib/api/hotel/audit";
...
export default function HotelAuditPage() {
  ...
  const fetchLogs = useCallback(async (currentFilters: typeof defaultFilters) => {
    ...
    setError(null);
    try {
      const data = await hotelAuditApi.list({
        page: 1,
        limit: 50,
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <PageHeader
          title="Audit Logs"
          description="Activity trail for your hotel operations."
          count={logs.length}
          countLabel="Records"
        />

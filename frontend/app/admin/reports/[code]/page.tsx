...
export default function AdminReportDetailPage() {
  ...
  const fetchReport = useCallback(async (code: string, filters?: ReportFilters) => {
    ...
    setError(null);
    try {
      const data = await adminReportsApi.get(code, filters);
      setDetail(data);
      setPage(1);
    } catch (err: unknown) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportData = await adminReportsApi.export(reportCode, {
        format: exportFormat,
      ...
      if (exportData.status === "pending" || exportData.status === "processing") {
        for (let attempt = 0; attempt < 60; attempt += 1) {
          await sleep(1000);
          const latest = await adminReportsApi.getExport(exportData.id);
          ...
          <DataToolbar
            ...
            actionsSlot={
              <>
                ...
                <PermissionGuard permission="admin:reports:export">

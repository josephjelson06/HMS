...
- `HMS_PERF_BASE_URL` (default `http://127.0.0.1:8000/api`)
- `HMS_PERF_ORIGIN` (default `http://localhost:3000`)
- `HMS_PERF_DURATION` (default `2m`)
- `HMS_PERF_VUS_ADMIN` (default `5`)
- `HMS_PERF_VUS_HOTEL_REFRESH` (default `10`)
- `HMS_PERF_VUS_HOTEL_CRUD` (default `10`)
- `HMS_PERF_THRESH_P95_REFRESH_MS` (default `250`)
- `HMS_PERF_THRESH_P95_CRUD_MS` (default `400`)

### Option A: Run k6 installed on host
```powershell
$env:HMS_PERF_BASE_URL="http://127.0.0.1:8000/api"
$env:HMS_PERF_ORIGIN="http://localhost:3000"
...
```powershell
$env:HMS_PERF_BASE_URL="http://host.docker.internal:8000/api"
$env:HMS_PERF_ORIGIN="http://localhost:3000"

docker run --rm `
  -e HMS_PERF_BASE_URL=$env:HMS_PERF_BASE_URL `
  -e HMS_PERF_ORIGIN=$env:HMS_PERF_ORIGIN `
  -e HMS_PERF_DURATION=2m `
  -e HMS_PERF_VUS_ADMIN=5 `
  -e HMS_PERF_VUS_HOTEL_REFRESH=10 `
  -e HMS_PERF_VUS_HOTEL_CRUD=10 `

...
from app.modules.hotel.settings.schemas import (
    SettingCreate,
    SettingListResponse,
    SettingOut,
    SettingUpdate,
...
from app.modules.hotel.settings.service import SettingsService
...
async def get_setting(
    ...
    setting = await service.get(current_user.tenant_id, setting_uuid)
    if not setting:
...
@router.post(
    ...
    dependencies=[Depends(require_permission("hotel:settings:create"))],
)
async def create_setting(
    ...
    try:
        setting = await service.create(
            current_user.tenant_id, payload, updated_by=current_user.id
        )
    except ValueError as exc:
...
@router.put(
    "/{setting_id}",
    response_model=SettingOut,
    dependencies=[Depends(require_permission("hotel:settings:update"))],
)
...
@router.delete(
    ...
    dependencies=[Depends(require_permission("hotel:settings:delete"))],
)

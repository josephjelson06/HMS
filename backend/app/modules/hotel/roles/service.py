...
class HotelRoleService:
    ...
    async def create(self, tenant_id: UUID, payload) -> Role:
    ...
    async def update(self, role: Role, tenant_id: UUID, payload) -> Role:
        if role.is_system:
    ...
    async def delete(self, role: Role, tenant_id: UUID) -> None:
        if role.is_system:
    ...
    async def _validate_permissions(self, permission_codes: list[str]) -> list[Permission]:
        if not permission_codes:
            return []
        stmt = select(Permission).where(
            Permission.code.in_(permission_codes), Permission.scope == "hotel"
        )
        result = await self.session.execute(stmt)

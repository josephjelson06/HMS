...
class HotelUserService:
    ...
    async def list_users(
        ...
        total = await self.session.scalar(
            select(func.count())
            .select_from(User)
            .where(User.user_type == "hotel", User.tenant_id == tenant_id)
        )
    ...
    async def create(self, tenant_id: UUID, payload) -> User:
        ...
        if (
            ...
            or (role.tenant_id is not None and role.tenant_id != tenant_id)
        ):
    ...
    async def update(self, user: User, tenant_id: UUID, payload) -> User:
